	...
	return (
		<div
			className={classNames(
				...
				{
					"bg-server-error-page-bg-light": theme === "light",
					"bg-server-error-page-bg-dark": theme === "dark",
				},
			)}
		>
			<Card styleClass={cardStyles}>
				<CardBody className="py-10 lg:py-15">
					<div className="text-center">
						<h1 className="text-h3 mb-4">{t("theme.error.server_error.title")}</h1>
						<p className="text-subtitle2 opacity-70">{t("theme.error.server_error.subtitle")}</p>
					</div>
					<img
						className="md:max-h-[25rem] md:max-w-[25rem] max-h-[15rem] max-w-[15rem]"
						src="/public/media/error-pages/server-error/vector.webp"
						alt="server-error-vector"
					/>
					<Button onClick={handleButtonClick}>{t("theme.error.server_error.button_text")}</Button>
				</CardBody>
			</Card>
		</div>
	);
